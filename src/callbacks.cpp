...
public:
  ...
  //    }
    	...
    	virtual void doo() {
		      while(NucleoResult != 8){ //Aspetto di arrivare
			         ros::spinOnce();
		      }
          //////////////////////////////////////////////
          Mess.data = 0;
          Mess.data |= (1u << 1); //2 Posizione mid
          NucleoPublisher->publish(Mess);

          while(NucleoResult != 16){
            ros::spinOnce();
          }
          /////////////////////////////////////////////
          //Controllo griffe
          Mess.data = 0;
          Mess.data |= (1u << 7); //128 Griffe chiuse
          NucleoPublisher->publish(Mess);
          //Wait result controllo griffe
          while(NucleoResult != 512){ //512 Griffe chiuse arrivate
            ros::spinOnce();
          }
          ...
          //////////////////////////////////////////
          Mess.data = 0;
          Mess.data |= (1u << 6); //64 Griffe aperte
          NucleoPublisher->publish(Mess);
          //Wait result controllo griffe
          while(NucleoResult != 256){ //256 Griffe aperte arrivate
            ros::spinOnce();
          }
          ...
          Mess.data |= (1u << 0); //1 Posizione down
          NucleoPublisher->publish(Mess);
          while(NucleoResult != 8){ //Arrivato in posizione di down
            ros::spinOnce();
          }
	...
	//Inizializzo dei puntatori per il client server e per la macchina a stati
	void initCallback(ros::Publisher* pub, rfsm::StateMachine* fsm){
		NucleoPublisher=pub;
		rfsm = fsm;
	}
